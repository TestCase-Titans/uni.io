...
const FAQ = [
  {
    question: "register",
    answer: "You can register for an event by clicking on the 'Register' button on the event details page and filling out the registration form."
  },
  {
    question: "cancel",
    answer: "Yes, you can cancel your registration by going to 'My Registrations' and clicking the 'Cancel' button next to the event you wish to cancel."
  },
  {
    question: "payment",
    answer: "We accept all major credit cards, PayPal, and bank transfers for event registrations."
  },
  {
    question: "refund",
    answer: "Yes, we offer a full refund if cancellation is made at least 7 days before the event. Please check our refund policy for more details."
  },
  {
    question: "hello",
  ...
  {
    question: "help",
    answer: "I can help you with event registration, cancellations, payments, and refunds. What would you like to know?"
  }
];
...
const Chatbot: React.FC = () => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { 
      text: "👋 Hello! I'm your event assistant. Here's what I can help with:\n\n" +
  ...
  const findBestMatch = (input: string): string | null => {
    ...
    const exactMatch = FAQ.find(item => 
      inputLower === item.question.toLowerCase()
    );
    ...
    if (faqMatch) {
      ...
      try {
        if (genAI) {
          ...
          const prompt = `You are a helpful assistant for a university event management platform. 
          ...
          if (!FAQ.some(item => item.question.toLowerCase() === inputMessage.toLowerCase())) {
            FAQ.push({
  ...
  return (
    <div className="fixed bottom-5 right-5 z-50">
      {/* Chat Window */}
      <div className={`absolute bottom-16 right-0 w-96 h-[600px] flex flex-col bg-background rounded-xl shadow-2xl overflow-hidden transition-all duration-300 ease-in-out border border-border ${
