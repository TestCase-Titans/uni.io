...
export const mockAuthApi = {
  async login(credentials: LoginRequest): Promise<AuthResponse> {
    ...
    if (!credentials.email || !credentials.password) {
      throw { message: 'Email and password are required', status: 400 } as ApiError
    }
    ...
    const mockUser = {
      ...
      role: credentials.email.includes('admin') ? 'admin' : 'student' as const
    }
  ...
  async signup(userData: SignupRequest): Promise<AuthResponse> {
    ...
    if (userData.password.length < 6) {
      throw { message: 'Password must be at least 6 characters', status: 400 } as ApiError
    }
